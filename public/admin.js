$(document).ready(function() {
    ...
    window.updateLead = function(id) {
		...
		let celular = row.find('td:nth-child(3)').text();
		let genero = row.find('td:nth-child(4)').text();
		let situacao = row.find('td:nth-child(5)').text();
    ...
    $("#editForm").on("submit", function(event) {
		...
		const genero = $("#editGenero").val();
		const situacao = $("#editSituacao").val();

        $.ajax({
            url: `http://localhost:3000/leads/${id}`,
            method: "PUT",
            data: { name: name, email: email, celular: celular, genero: genero, situacao: situacao },
            success: function() {
                loadLeads();
